...
export default function Captcha({ onChange, lang = "tr", action = "form_submit", resetKey = 0 }) {
  ...
  // V3: küçük bir açıklama yeterli
  if (MODE === "v3") {
    return <div className="text-[12px] text-gray-500 -mt-2">Protected by reCAPTCHA.</div>;
  }

  // V2: her durumda clip alanı renderla; hata varsa koyu fallback göster
  const errorMsg =
    err === "missing-sitekey"
