...
// Credentials login için güvenli proxy.
...
const ACCOUNT_LOCK_DURATION_MS = 15 * 60_000; // 15 dk
...
// CSRF yanıtındaki next-auth çerezlerini topla -> "a=b; c=d" formatı
...
export async function POST(req) {
  ...
  try {
    ...
    // Railway/Proxy arkasında doğru origin:
    const envUrl = process.env.NEXTAUTH_URL; // zorunlu: https://<domain>
    const scheme = req.headers.get("x-forwarded-proto") || "http";
    const host = req.headers.get("host");
    const origin = envUrl || req.nextUrl?.origin || `${scheme}://${host}`;
    ...
    if (!csrfRes.ok) {
      return NextResponse.json(
        { success: false, message: msg.fail, reason: `csrf_${csrfRes.status}` },
        { status: 500 }
      );
    }
    ...
    if (!nextAuthCsrfToken) {
      return NextResponse.json(
        { success: false, message: msg.fail, reason: "no_csrf_token" },
        { status: 500 }
      );
    }
    ...
    let cbJson = {};
    const ct = cbRes.headers.get("content-type") || "";
      ...
      return NextResponse.json(
        {
          success: false,
          message: msg.fail,
          reason: cbJson?.error || `status_${cbRes.status}`,
        },
        { status: 401 }
      );
    }
    ...
    // (opsiyonel) console.error("[/api/login] unhandled:", e);
    return NextResponse.json(
      { success: false, message: MESSAGES[ pickLocale(req) ].fail, reason: "unhandled" },
      { status: 500 }
    );
  }
}
