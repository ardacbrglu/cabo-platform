// File: src/app/api/shopify_verify/[[...path]]/route.js
export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/**
 * Security Docblock
 * - App Proxy istekleri Shopify tarafından query'ye `signature` + `timestamp` eklenerek forward edilir.
 *   Bunlar OAuth client secret (SHOPIFY_*_SECRET) ile doğrulanır. (Frontende secret sızmaz.)
 * - S2S (server-to-server) istekleri JSON body ile gelir ve merchant-özel `hmacSecret` ile imzalanır.
 * - İki mod da TLS (HTTPS) üstünden çalışır. Proxy Set-Cookie taşımaz; cookie'yi tema JS set eder.
 * - Rate limit, timestamp tazelik ve dedup ile suistimal azaltılır.
 */
...
/** Shopify App Proxy imzası doğrulaması (query.signature) */
...
// --- Ortak iş: link bul, host doğrula, dedup + click yaz
...
// ---- Mod 1: App Proxy (storefront → /apps/cabo[/verify])
async function handleProxy(req) {
  ...
  // shopDomain'i header'dan al; yoksa query/body fallback
  const shopDomain =
    (req.headers.get('x-shopify-shop-domain') || '').toLowerCase() ||
    (url.searchParams.get('shop') || '').toLowerCase();

  // Body opsiyoneldir; GET de gelebilir
  let body = null;
  try {
...
// ---- Mod 2: S2S (Remix/başka sunucu → /api/shopify_verify (JSON, HMAC=merchant secret))
...
export async function GET(req) {
  ...
  return hasProxySig ? handleProxy(req) : json({ ok: true, route: 'root' });
}

export async function POST(req) {
  const url = new URL(req.url);
  const hasProxySig = url.searchParams.has('signature');
  // App Proxy ile geldiyse önce onu dene; değilse S2S
  return hasProxySig ? handleProxy(req) : handleS2S(req);
}
