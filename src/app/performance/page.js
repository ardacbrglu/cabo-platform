...
import {
  BarChart2, ShoppingCart, Download, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export default function PerformancePage() {
  ...
  const [chartHover, setChartHover] = useState({ index: null, x: 0, y: 0 });
  ...
  // Grafik datası — Günlük gridli, kareli arka plan, tooltip hover!
  const chartData = useMemo(() => {
    const selectedIds =
      filters.productIds.includes(0) || filters.productIds.length === 0
        ? null
        : filters.productIds;
      ...
      datasets: [
        {
          ...
          backgroundColor: selectedChart === 'clicks'
            ? 'rgba(129, 215, 66, 0.13)'
            : 'rgba(209, 255, 208, 0.09)',
          borderWidth: 3,
          tension: 0.36,
          fill: true,
          pointRadius: 5,
  ...
  // Grafik üstü: seçili ürün adı (tek ürünse adı, yoksa All Products)
  const selectedProductName = useMemo(() => {
    if (
      filters.productIds.length === 0 ||
      filters.productIds.includes(0)
    ) return t('performance.all_products');
  ...
  // Grafik ayarları — kareli grid, x etiketi yok, sadece hover tooltip!
  const chartOptions = {
    responsive: true,
  ...
  const PANEL_HEIGHT = typeof window !== "undefined" && window.innerWidth < 650 ? 'auto' : 600;
  const KPI_CARD_HEIGHT = 66;
        ...
        <section className="flex-1 min-w-[320px] flex flex-col gap-5">
          ...
          <div className="bg-[#181818] rounded-2xl shadow-lg p-5 flex-1 flex flex-col h-full justify-between relative min-h-[320px]">
            <div className="flex flex-row items-center mb-4 gap-3">
              <div className="text-lg font-extrabold text-[#d1ffd0] tracking-wide flex-shrink-0">
                {selectedProductName}
              </div>
            ...
            <div className="w-full flex-1 flex items-end pb-1 min-h-[220px] relative">
              ...
              {/* Grid + kareli arka plan için ek style burada! */}
              <style jsx global>{`
                .chartjs-render-monitor {
                  background: repeating-linear-gradient(
                    to right, #1d201c 0, #1d201c 1px, transparent 1px, transparent 42px
                  ), repeating-linear-gradient(
                    to bottom, #1d201c 0, #1d201c 1px, transparent 1px, transparent 38px
                  );
                  border-radius: 18px;
                }
              `}</style>
        ...
        <aside className="w-full lg:w-[370px] flex flex-col">
          <div
            ...
            style={{
              ...
              boxShadow: '0 0 6px #132e1248',
              overflow: 'hidden',
              minHeight: 330
            }}
          >
            <div className="text-2xl md:text-3xl font-bold text-[#d1ffd0] mb-2 mt-7 text-center font-sans">{t('performance.confirmed_sales')}</div>
            {allConfirmedSales.length === 0 ? (
              ...
              <ul
                className="flex-1 px-3 pt-3 overflow-y-auto"
                style={{ maxHeight: SALES_PANEL_BODY, minHeight: SALES_PANEL_BODY }}
              >
                {pagedSales.map((s, i) => {
