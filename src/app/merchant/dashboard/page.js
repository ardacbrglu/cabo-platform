...
export default function MerchantDashboardPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setForm({ name: "", description: "", image_url: "", price: "", commission_rate: "", merchant_url: "", max_sales_limit: "" });
        fetchProducts();
  ...
  const toggleShowCode = (product_id) => setShowCode((prev) => ({ ...prev, [product_id]: !prev[product_id] }));
  const copyProductCode = async (product_id, code) => {
    try {
      await navigator.clipboard.writeText(code);
      setCopyMsg((prev) => ({ ...prev, [product_id]: t("copied") }));
  ...
  const saveEdits = async () => {
    if (Number(editValues.commission_rate) < minCommission) {
      ...
      {formVisible && (
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <input type="number" className={inputClass} placeholder={t("commissionRate")} required step="0.1" min={minCommission} onChange={e => setForm({ ...form, commission_rate: e.target.value })} />
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-wrap justify-between text-base mb-2 text-gray-200 font-mono gap-y-1">
                ...
                <span>
                  <span className="text-gray-500">{t("commission")}</span>: <span className="font-bold text-green-300">{Number(p.commission_rate).toFixed(2)}%</span>
                </span>
              </div>
              <div className="flex flex-wrap justify-between text-xs mb-2 text-gray-400 gap-y-1">
                <span>{t("clicks")}: <b>{p.total_clicks}</b></span>
