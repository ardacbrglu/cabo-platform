...
export default function MerchantPaymentsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await apiFetch("/api/platform_info", { method: "GET" });
        const j = await r.json().catch(() => ({}));
        setPlatformBank(j || null);
      } catch {
  ...
  return (
    <MerchantLayout>
      ...
      <div className="flex items-center justify-between mb-7 gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-[#85a48b] text-base ml-3">
            Manage and review all commission payouts to your affiliates.
          </span>
        </div>
        ...
        <button
          ...
          title="Refresh"
          className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[#2a2a2a] bg-[#1a1a1a] text-[#caffb6] hover:bg-[#222] disabled:opacity-60"
          disabled={reloading || paying}
        >
          {reloading ? <Loader2 size={18} className="animate-spin" /> : <RotateCcw size={18} />}
      ...
      {/* notice */}
      {notice && (
        <div className="mb-4 bg-[#1e2a1f] text-[#d1ffd0] border border-[#2b3b2d] px-4 py-3 rounded">
          {notice}
        </div>
      )}
      ...
      {showPayPopup && (
        <div className="fixed inset-0 z-40 bg-black/50 flex items-center justify-center">
          <div className="bg-[#161a16] rounded-2xl p-8 shadow-xl min-w-[350px] max-w-sm w-full border border-[#344f29]">
            ...
            <PlatformBankInfo platformBank={platformBank} />
            <div className="text-lg text-[#afffb2] font-bold mb-4">
              Total Amount:{" "}
              {formatAmount(
                items
                  .filter((i) => i.itemIds.some((id) => selected.includes(id)))
          ...
          <div className="bg-[#181818] rounded-lg shadow-lg p-7 w-[670px] max-w-full relative font-mono border border-[#232323]">
            ...
            >
              <svg width="24" height="24">
                <path fill="currentColor" d="M6 6L18 18M6 18L18 6" />
              </svg>
            </button>
            ...
            <div className="mb-2 text-xs text-gray-400 flex flex-wrap items-center gap-x-5 gap-y-1">
              <span>
                <span className="font-bold text-[#d1ffd0]">Affiliate:</span> {detailsAffiliate}
              </span>
              <span>
                <span className="font-bold text-[#d1ffd0]">Request Date:</span> {detailsMeta?.requestDate || "-"}
              </span>
              <span>
                <span className="font-bold text-[#d1ffd0]">Status:</span>{" "}
              ...
              <span>
                <span className="font-bold text-[#d1ffd0]">Total:</span>{" "}
                <span className="text-[#81d742] font-bold">{formatAmount(detailsMeta?.total || 0)}</span>
              </span>
            </div>
              ...
              <table className="min-w-full text-xs font-mono">
                <thead>
                  <tr className="text-[#baffc1] border-b border-[#232323]">
                    ...
                    <th className="py-2 px-3">Has Link</th>
                  </tr>
                </thead>
                <tbody>
                  {detailsLoading && (
                  ...
                  {detailsData.map((sale, idx) => (
                    <tr key={idx} className="border-b border-[#232323] group">
                      ...
                      <td className="py-2 px-3 text-right" style={{ color: "#81d742" }}>
                        {formatAmount(sale.commission)}
                      </td>
                      <td className="py-2 px-3 text-right">{sale.quantity}</td>
                      <td className="py-2 px-3">{sale.sale_date || "-"}</td>
                      <td className="py-2 px-3">
                        <span className={`font-bold px-2 py-1 rounded border border-[#333] bg-[#232323]/60 text-xs ${getSalestatusStyle(sale.status)}`}>
                          {sale.status}
                        </span>
                      </td>
                      <td className="py-2 px-3 text-center">{sale.has_affiliate_link ? "✓" : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function PlatformBankInfo({ platformBank }) {
  return (
    <div className="mb-4">
