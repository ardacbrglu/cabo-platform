...
const statusStyles = {
  pending: "bg-yellow-900/60 text-[#ffe7a1]",
  merchant_paid: "bg-green-900/60 text-[#b2ffa6]",
  platform_confirmed: "bg-cyan-900/60 text-[#a6fffb]",
  rejected: "bg-red-900/60 text-[#f3aaaa]",
};
...
// Status badge için arka plan/text rengi (tablo ve modal için)
...
export default function MerchantPaymentsPage() {
  ...
  const [detailsAmount, setDetailsAmount] = useState(0);
  ...
  async function handleMakePayment() {
    ...
    setDetailsStatus(item.status);
    setDetailsAmount(item.amount);
    setDetailsDate(item.requested_at?.replace("T", " ").substring(0, 19) || "");
    try {
      ...
      if (data.details && Array.isArray(data.details)) {
        ...
        setDetailsMeta(data.meta); // <-- BUNU BURADA SET ET!
      } else {
        setDetailsData([]);
        setDetailsMeta(null);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#101010] text-white font-sans tracking-tight">
      ...
      <header className="flex justify-between items-center px-10 py-6 bg-[#111] border-b border-[#1f1f1f] shadow-sm">
        ...
        <nav className="flex gap-8 items-center text-sm font-medium">
          <Link
            ...
            className={`transition hover:text-[#81d742] hover:scale-[1.015] ${pathname === "/merchant/dashboard"
          ...
          <Link
            ...
            className={`transition hover:text-[#81d742] hover:scale-[1.015] ${pathname === "/merchant/merchant_payments"
              ? "text-[#81d742] font-semibold"
              : "text-gray-200"
              }`}
          >
          ...
          <Link
            ...
            className={`transition hover:text-[#81d742] hover:scale-[1.015] ${pathname === "/merchant/info"
              ? "text-[#81d742] font-semibold"
              : "text-gray-200"
              }`}
          >
          ...
          <Link
            ...
            className={`transition hover:text-[#81d742] hover:scale-[1.015] ${pathname === "/merchant/support"
              ? "text-[#81d742] font-semibold"
              : "text-gray-200"
              }`}
          >
      ...
      <main className="max-w-5xl mx-auto mt-10 px-2 pb-24 flex-grow">
