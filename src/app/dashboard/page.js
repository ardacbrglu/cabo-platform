...
export default function Dashboard() {
  ...
  // Auth/RBAC yönlendirmesi — UserContext üzerinden (race yok)
  useEffect(() => {
    if (!ready) return;
    if (!isAuthenticated) {
  ...
  // Dashboard verileri (yalnızca hazır + auth iken)
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        if (!res.ok) {
          // 401/403 vs geldi ise login’e al
          router.replace("/login");
          return;
        }
        ...
        // UserContext’e temel bilgiler (opsiyonel)
        setUser((u) => ({
          ...(u || {}),
          name: data.username || "",
          email: data.email || "",
        ...
        // Ağ hatasında sessiz kal — periyodik denemeler devam etsin
      }
    };
            ...
            <section className="flex w-full gap-5">
              <div className="flex flex-col gap-5 flex-[2]">
                ...
                <div className="bg-[#181818] rounded-xl shadow flex flex-col items-center py-5 px-6">
                  ...
                  <div
                    className="mt-2 text-xs font-bold animate-pulse font-mono"
              ...
              <div className="flex flex-col gap-5 flex-1">
                ...
                <div className="bg-[#181818] rounded-xl shadow flex flex-col items-center py-6 px-6">
                  ...
                  {payoutstatus && payoutstatus !== "success" && (
                    <div className="mt-2 text-red-400 text-xs text-center font-mono">{payoutstatus}</div>
                  )}
