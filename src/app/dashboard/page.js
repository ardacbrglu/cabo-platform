...
// Kullanıcı ajanından cihaz tipi (Frontend fallback için, backend de aynı şekilde göndermeli)
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      // lastClick cihaz tespiti frontend fallback (eğer backend göndermediyse)
    ...
    fetchStats();
    interval = setInterval(fetchStats, 8000); // 8 sn'de bir canlı veri
  ...
  return (
    <Layout>
      <main className="flex flex-col items-center w-full max-w-7xl mx-auto flex-1 justify-center mt-5 gap-8 px-4">
        {/* Banka uyarı barı */}
        {!loading && (ibanMissing || bankMissing || realNameMissing) && (
          <div className="w-full max-w-2xl mx-auto bg-red-900/80 text-red-200 font-mono rounded-xl px-6 py-3 text-sm text-center mb-3 border border-red-700 shadow animate-pulse z-20">
