 ...
 * - AuthZ: RBAC server-side (requireRole('affiliate')), status: 'active'. Frontend yalnız UX için soft-guard yapar.
 * - CSRF: Sadece GET kullanır; mutasyon yok. Tüm çağrılar merkezi apiFetch ile (credentials:include, X-Requested-With, X-Request-Id).
 * - Ratelimit: Polling MIN_INTERVAL_MS >= 12s → GET 60/dk sınırına uyum. 429 → Retry-After’a göre bekleme.
 * - Privacy: AbortController + visibilitychange → gereksiz trafik ve sızıntı yok.
 * - Errors: 401→/login, 403→/unauthorized; diğer hatalarda backoff.
 */
...
export default function Dashboard() {
  ...
  // Role guard (UX only; server tarafı kesin)
  useEffect(() => {
    if (!isReady) return;
      ...
      try {
        ...
        if (res.status === 429) {
          ...
          try {
            ...
            retryBody = Number(j?.retry_after) || 0;
          } catch {}
  ...
  }, [isReady, setUser, router]); // setUser stable (UserContext’te useCallback)
          ...
          <section className="grid w-full grid-cols-12 gap-7">
            ...
            <div className="col-span-8 bg-[#181818] rounded-xl shadow flex flex-col items-center py-4 px-5 min-h-[240px] overflow-hidden">
              ...
              <div className="w-full max-w-sm mt-3">
                <div className="text-[13px] font-extrabold mb-2 font-mono text-center" style={{ color: COLOR_CABO }}>
                  {t("payoutRequest")}
                </div>
                <button
